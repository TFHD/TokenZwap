...
export async function swap(tokenInIsA: boolean, amount: string, minOut: bigint) {
	await runTx('Swap confirmed.', async () => {
		...
		if (minOut <= 0n) throw new Error('Quote is too small for this slippage.');
		await ensureAllowance(tokenInIsA ? tokenA : tokenB, await signer.getAddress(), parsed);
		return (await amm.swap(
			tokenInIsA ? pool.tokenA : pool.tokenB,
			parsed,
			minOut,
			txDeadline()
...
export async function addLiquidity(amountA: string, amountB: string) {
	await runTx('Liquidity added.', async () => {
		...
		return (await amm.addLiquidity(parsedA, parsedB, txDeadline())) as TransactionResponse;
	});
}
...
export async function removeLiquidity(lpAmount: string) {
	await runTx('Liquidity removed.', async () => {
		...
		return (await amm.removeLiquidity(parsed, txDeadline())) as TransactionResponse;
	});
}
